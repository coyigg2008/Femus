...
//   }
  ...
  FieldSplitTree VelPf(PREONLY, MLU_PRECOND, fieldVelPf, solutionTypeVelPf, "VelPf");
  ...
  FieldSplitTree DispPs(PREONLY, MLU_PRECOND, fieldDispPs, solutionTypeDispPs, "DispPs");
  ...
  //FieldSplitTree FSI(RICHARDSON, FIELDSPLIT_PRECOND, FS2, "FSI");
  FieldSplitTree FSI(PREONLY, FIELDSPLIT_MULTIPLICATIVE_PRECOND, FS2, "FSI");
  FSI.SetRichardsonScaleFactor(.4);
  
  //END buid fieldSplitTree
  
  system.SetOuterSolver(GMRES);//SetOuterKSPSolver("lgmres");
  //system.SetLinearEquationSolverType (FEMuS_FIELDSPLIT, INCLUDE_COARSE_LEVEL_TRUE);
  system.SetLinearEquationSolverType (FEMuS_DEFAULT, INCLUDE_COARSE_LEVEL_TRUE);   
  ...
  //if(dim==2){
    system.SetTolerances(1.e-10, 1.e-9, 1.e+50, 40000, 40);
  //}
  //else{
