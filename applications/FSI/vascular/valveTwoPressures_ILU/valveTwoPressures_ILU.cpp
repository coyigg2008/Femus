...
//   }
  ...
  if(iproc == 0){
    ...
    sprintf(stdOutputName, "stdoutput_level%d_nprocs%d_stiffness5ILU.txt",numberOfUniformRefinedMeshes, nprocs);
...
void PrintConvergenceInfo(char *stdOutfile, const unsigned &level, const int &nprocs){
  ...
  sprintf(outFileName, "valve2D_convergence_level%d_nprocs%d_stiffness5ILU.txt",level, nprocs);
